...
async function appendSourceAndRefreshEvent(
  ...
  await prisma.$transaction([
    prisma.weatherAlertSource.create({
      data: {
        eventId,
    ...
    prisma.weatherAlertEvent.update({
      where: { id: eventId },
...
export async function ingestWeatherAlertItem(
  ...
  if (existing) {
    await appendSourceAndRefreshEvent(existing.id, source, item, rawPayload);
  ...
  try {
    ...
    const raced = await prisma.weatherAlertEvent.findUnique({
      where: { eventType_sourceEventId: lookupKey },
    });
    if (!raced) {
      throw error;
    ...
    await appendSourceAndRefreshEvent(raced.id, source, item, rawPayload);
